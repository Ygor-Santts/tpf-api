...
export class IGetWorkersByParametersResponseDTO {
  ...
  @ApiProperty() operationCities!: { id: number; name: string }[];
  ...
  constructor(worker: IWorker, ratingSummary?: { average: number; count: number }) {
    ...
    this.jobCategories = worker.jobOccupations.reduce(
      (acc, occupation) => {
        ...
        if (!categoryGroup) {
          categoryGroup = { id: category.id, name: category.name, occupations: [] };
          acc.push(categoryGroup);
        }
        categoryGroup.occupations.push({ id: occupation.id, name: occupation.name });
        return acc;
      },
      [] as { id: number; name: string; occupations: { id: number; name: string }[] }[],
    );
...
export class IGetWorkersByParametersPaginatedResponseDTO extends PaginationResponseDTO<IGetWorkersByParametersResponseDTO> {
  @ApiProperty({ type: IGetWorkersByParametersResponseDTO, isArray: true })
...
export class GetWorkersByParametersPaginated {
  ...
  ): Promise<IPaginationResponseDTO<IGetWorkersByParametersResponseDTO> | HttpException> {
    const { page, limit, ...parameters } = dto;

    const [workers, count] = await this.workerRepository.getWorkersByParametersPaginated(
      { ...parameters, page, limit },
      ['user', 'operationCities', 'jobOccupations', 'jobOccupations.category'],
    );

    if (!workers.length) {
      return new NotFoundException('Nenhum trabalhador encontrado com os parâmetros fornecidos');
    }

    const workerIds = workers.map((w) => w.id);
    const ratingsMap = await this.workerRepository.getRatingsByWorkerIds(workerIds);

    const data = workers.map(
      (worker) => new IGetWorkersByParametersResponseDTO(worker, ratingsMap.get(worker.id)),
    );
